...
__global__ void blob_rearrange_kernel2(const Dtype* in, Dtype* out, int num, int channels, int width, int height, int widthheight, int padding, int padshift, int pwidthheight, int pkernel)
{
  ...
  Dtype *bottom0diff, const Dtype *bottom1, const Dtype *topdiff) 
{
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int m = (index / bottomchannels / bottomwidth) % bottomheight + kernel_radius; //h-pos
    ...
    const int bot0index = ((n * bottomheight) + (m-kernel_radius)) * bottomwidth + (l-pad_size);
  ...
  const Dtype *bottom0, Dtype *bottom1diff, const Dtype *topdiff) 
{
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    const int sumelems = (kernel_radius*2+1)*(kernel_radius*2+1)*bottomchannels;
    const int bot1index = ((n * bottomheight) + (m-kernel_radius)) * bottomwidth + (l-pad_size);
  ...
  Dtype *bottom0diff, const Dtype *bottom0, const Dtype *bottom1, const Dtype *topdiff) 
{
  CUDA_KERNEL_LOOP(index, nthreads) {
    int l = index % bottomwidth + pad_size; //w-pos
    int m = (index / bottomwidth) % bottomheight + kernel_radius; //h-pos
    int n = (index / bottomwidth / bottomheight) % bottomchannels; //channels
      ...
      const vector<Blob<Dtype>*>& top)
{
    ...
    const int pwidthheight = (bwidth + 2 * pad_size_) * (bheight + 2 * kernel_radius_);
    
    corr1d::blob_rearrange_kernel2<Dtype><<<totalBlocksRearr,threads_per_block>>>
            (bottom[0]->gpu_data(),rbot1_->mutable_gpu_data(),bnum,bchannels,bwidth,bheight,bwidthheight,pad_size_,0,pwidthheight,kernel_radius_);
    
    corr1d::blob_rearrange_kernel2<Dtype><<<totalBlocksRearr,threads_per_block>>>
            (bottom[1]->gpu_data(),rbot2_->mutable_gpu_data(),bnum,bchannels,bwidth,bheight,bwidthheight,pad_size_,pad_shift_,pwidthheight,kernel_radius_);
    
    const int num = bnum;
    const int channels = bchannels;
    const int height = bheight + 2*kernel_radius_;
      ...
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
    ...
    const int paddedheight = height + 2*kernel_radius_;
