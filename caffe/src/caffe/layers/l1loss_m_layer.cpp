...
#include "caffe/layers/l1_loss_m_layer.hpp"
...
void L1LossMLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
                                    const vector<Blob<Dtype> *> &top)
{
  LossLayer<Dtype>::LayerSetUp(bottom, top);
  //  min_diff_ = this->layer_param_.l1_loss_param().min_diff();
  if (bottom.size() == 1)
  {
    diff_top_vec_.clear();
  ...
  else if (bottom.size() == 3)
  {
    ...
    diff_param.mutable_eltwise_param()->set_operation(EltwiseParameter_EltwiseOp_SUM);
    diff_layer_.reset(new EltwiseLayer<Dtype>(diff_param));
    diff_layer_->SetUp(bottom_1_2, diff_top_vec_);
  }
  else
  {
    LOG(FATAL) << "L1LossMLayer needs one or two or three input blobs.";
  }
...
template <typename Dtype>
void L1LossMLayer<Dtype>::Reshape(
...
template <typename Dtype>
void L1LossMLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
                                     const vector<Blob<Dtype> *> &top)
{
  NOT_IMPLEMENTED;
}
...
template <typename Dtype>
void L1LossMLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
...
#ifdef CPU_ONLY
STUB_GPU(L1LossMLayer);
#endif

INSTANTIATE_CLASS(L1LossMLayer);
REGISTER_LAYER_CLASS(L1LossM);
