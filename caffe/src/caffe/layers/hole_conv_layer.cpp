...
void HoleConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  HoleConvolutionParameter conv_param = this->layer_param_.hole_conv_param();
  ...
  num_output_ = this->layer_param_.hole_conv_param().num_output();
  CHECK_GT(num_output_, 0);
  group_ = this->layer_param_.hole_conv_param().group();
  ...
  bias_term_ = this->layer_param_.hole_conv_param().bias_term();
  if (this->blobs_.size() > 0) {
    ...
    //if (Caffe::getThreadId() == 0) {
      shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
          this->layer_param_.hole_conv_param().weight_filler()));
    ...
    if (bias_term_) {
     ...
     // if (Caffe::getThreadId() == 0) {
        shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
            this->layer_param_.hole_conv_param().bias_filler()));
