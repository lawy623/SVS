...
#include "caffe/layers/correlation_deploy.hpp"
...
template <typename Dtype>
void CorrelationDeployLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void CorrelationDeployLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
                                        const vector<Blob<Dtype> *> &top)
{
  ...
  temp1_.Reshape(num_, paddedbottomheight, paddedbottomwidth, bottomchannels);
  temp2_.Reshape(num_, paddedbottomheight, paddedbottomwidth, bottomchannels);
  mean_.Reshape(num_, paddedbottomheight, paddedbottomwidth, 1);
  variance_.Reshape(num_, paddedbottomheight, paddedbottomwidth, 1);
  temp_.Reshape(num_, paddedbottomheight, paddedbottomwidth, bottomchannels);
  sum_multiplier_.Reshape(1, 1, 1, bottomchannels);
...
template <typename Dtype>
void CorrelationDeployLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
                                            const vector<Blob<Dtype> *> &top)
{
  ...
  // time_t t = clock();
  if(if_mvn_) {
      blob_rearrange_kernel2<Dtype>(bottom[0]->cpu_data(), temp1_.mutable_cpu_data(), bnum, bchannels, bwidth, bheight, bwidthheight, 0, 0, bwidthheight, kernel_radius_);
      blob_rearrange_kernel2<Dtype>(bottom[1]->cpu_data(), temp2_.mutable_cpu_data(), bnum, bchannels, bwidth, bheight, bwidthheight, pad_size_, pad_shift_, pwidthheight, kernel_radius_); 
      int nnum = temp1_.num() * temp1_.channels() * temp1_.height();
      int ddim = temp1_.width();
      const Dtype* iin = temp1_.cpu_data();
      Dtype* oout = rbot1_->mutable_cpu_data();
      caffe_cpu_gemv<Dtype>(CblasNoTrans, nnum, ddim, 1. / ddim, iin,
                            sum_multiplier_.cpu_data(), 0., mean_.mutable_cpu_data());// EX
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, nnum, ddim, 1, -1.,
                            mean_.cpu_data(), sum_multiplier_.cpu_data(), 0.,
                            temp_.mutable_cpu_data());
      caffe_add(temp_.count(), iin, temp_.cpu_data(), oout);
      caffe_powx(nnum * ddim, oout, Dtype(2), temp_.mutable_cpu_data());
      caffe_cpu_gemv<Dtype>(CblasNoTrans, nnum, ddim, 1. / ddim, temp_.cpu_data(),
                            sum_multiplier_.cpu_data(), 0.,
                            variance_.mutable_cpu_data());
      caffe_powx(variance_.count(), variance_.cpu_data(), Dtype(0.5),
                 variance_.mutable_cpu_data());
      caffe_add_scalar(variance_.count(), Dtype(1e-9), variance_.mutable_cpu_data());
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, nnum, ddim, 1, 1.,
                 variance_.cpu_data(), sum_multiplier_.cpu_data(), 0.,
                 temp_.mutable_cpu_data());
      caffe_div(temp_.count(), oout, temp_.cpu_data(), oout);
      iin = temp2_.cpu_data();
      oout = rbot2_->mutable_cpu_data();
      caffe_cpu_gemv<Dtype>(CblasNoTrans, nnum, ddim, 1. / ddim, iin,
                            sum_multiplier_.cpu_data(), 0., mean_.mutable_cpu_data());// EX
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, nnum, ddim, 1, -1.,
                            mean_.cpu_data(), sum_multiplier_.cpu_data(), 0.,
...
template <typename Dtype>
void CorrelationDeployLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
...
#endif
INSTANTIATE_CLASS(CorrelationDeployLayer);
REGISTER_LAYER_CLASS(CorrelationDeploy);
