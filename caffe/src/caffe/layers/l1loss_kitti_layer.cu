...
#include "caffe/layers/l1_loss_kitti_layer.hpp"
...
__global__ void FindNotNaNs(const int n, const Dtype* in, Dtype* out) {
  CUDA_KERNEL_LOOP(index, n) {
      ...
      out[index] = abs(in[index])>=1.0 ? Dtype(1) : Dtype(0); // For Kitti data
  }
} 
...
template <typename Dtype>
void L1LossKittiLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top)
{
  ...
  // FindNotNaNs<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, diffptr->gpu_data(), mask_.mutable_gpu_data());
  FindNotNaNs<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, bottom[1]->gpu_data(), mask_.mutable_gpu_data());

  cudaDeviceSynchronize();
  CUDA_POST_KERNEL_CHECK;

#ifdef DEBUG_MASK    /// DEBUG Mask
  Mat mask_image(bottom[0]->height(), bottom[0]->width(), CV_32FC1);
  cudaMemcpy(mask_image.data, mask_.gpu_data(), count/num*sizeof(Dtype), cudaMemcpyDefault);
  Mat disp_image(bottom[0]->height(), bottom[0]->width(), CV_32FC1);
  cudaMemcpy(disp_image.data, bottom[1]->gpu_data(), count/num*sizeof(Dtype), cudaMemcpyDefault);
  string filename, dispname;
  cout << "Enter mask image name to save: ";
  cin >> filename;
  cout << "Enter disp image name to save: ";
  cin >> dispname;
  imwrite(filename.c_str(), mask_image*255);
  imwrite(dispname.c_str(), -disp_image);
...
template <typename Dtype>
void L1LossKittiLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(L1LossKittiLayer);
