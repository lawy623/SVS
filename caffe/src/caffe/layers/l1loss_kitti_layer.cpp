...
void L1LossKittiLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if(bottom.size() == 1) {
    ...
    diff_layer_->SetUp(bottom, diff_top_vec_);
      ...
      LOG(FATAL) << "L1LossKittiLayer needs one or two input blobs.";
  }
...
template <typename Dtype>
void L1LossKittiLayer<Dtype>::Reshape(
...
template <typename Dtype>
void L1LossKittiLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void L1LossKittiLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  NOT_IMPLEMENTED;
}

#ifdef CPU_ONLY
STUB_GPU(L1LossKittiLayer);
#endif

INSTANTIATE_CLASS(L1LossKittiLayer);
REGISTER_LAYER_CLASS(L1LossKitti);
