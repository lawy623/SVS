...
// [Jiahao edited] matrix column normalization kernel for GPU
template <typename Dtype>
__global__ void col_normalization_gpu_kernel(const int rows, const int cols, Dtype* data_col) {
  CUDA_KERNEL_LOOP(index, cols) {
    Dtype std = 0;
    Dtype mean = 0;
    Dtype* data_col_ptr = data_col + index;
    for (int i=0; i < rows; i++) { // accumulate the statistics
      std += (*data_col_ptr) * (*data_col_ptr);
      mean += *data_col_ptr;
      data_col_ptr += cols;
    }
    mean = mean / rows;
    std = sqrt(std / rows - mean * mean)+STD_EPS; // compute the standard deviation
    data_col_ptr = data_col + index;
    for (int i=0; i < rows; i++) {
      *data_col_ptr = ((*data_col_ptr) - mean) / std;
      data_col_ptr += cols;
    }
  }
}    

// [Jiahao edited] matrix column normalization
template <typename Dtype>
void col_normalization_gpu(const int rows, const int cols, Dtype* data_col) {
  // launch cols kernels, each compute a norm value
  col_normalization_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(cols), 
      CAFFE_CUDA_NUM_THREADS>>>(rows, cols, data_col);
  CUDA_POST_KERNEL_CHECK;
}

// Explicit instantiation
template void col_normalization_gpu<float>(const int row, const int cols, float* data_col);
template void col_normalization_gpu<double>(const int row, const int cols, double* data_col);
